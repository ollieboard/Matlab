function [ max_earning, max_evaluation, bestDays ] =...
    evaluateCrossingAverage( stockdata, min_days, max_days )
    %evaluates the stocks earning potential according to moving average
    %buy/sell in 5 - 200 days starting at startdate and ending at enddate. 
    %Buying or selling starts X days after startdate.
    ...
    for days = min_days:1:max_days
        ...
        for i = days+1:1:length(stockdata)
            %calculate moving average of previous data:
            average = mean(stockdata(i-days:i-1,5));
